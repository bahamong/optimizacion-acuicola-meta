...
const COLOR_OPTIMA = '#d400ff'  // ruta óptima (Dijkstra) — color vivo y llamativo
...
function Leyenda() {
  ...
  const rutas = [
    ...
    { color: COLOR_OPTIMA, dash: true,  label: 'Ruta óptima — Dijkstra' },
  ]
  ...
  if (minimizada) {
    return (
      <button
        ...
        className="absolute bottom-3 left-3 z-[1000] bg-slate-900/95 border border-slate-700 rounded-xl px-3 py-2 text-xs text-white flex items-center gap-2 hover:bg-slate-800 transition-colors shadow-lg"
        title="Mostrar leyenda"
      >
        <FaLayerGroup /> Leyenda <FaChevronUp />
      </button>
    )
  }
  ...
  return (
    <div className="absolute bottom-3 left-3 z-[1000] bg-slate-900/95 border border-slate-700 rounded-xl p-3 text-xs text-white min-w-[180px]">
      <div className="flex items-center justify-between mb-1.5">
        <p className="font-bold text-slate-300">Nodos</p>
        <button
          onClick={() => setMinimizada(true)}
          className="text-slate-400 hover:text-white -mt-1 -mr-1 p-1"
      ...
      <MapContainer center={centro} zoom={6} className="w-full h-full" scrollWheelZoom zoomControl>
        ...
        {caminoDijkstra.length > 1 && <>
          ...
          <Polyline positions={caminoDijkstra}
            pathOptions={{ color: COLOR_OPTIMA, weight: 14, opacity: 0.25, lineCap: 'round', lineJoin: 'round' }} />
          {/* Línea base brillante */}
          <Polyline positions={caminoDijkstra}
            pathOptions={{ color: COLOR_OPTIMA, weight: 7, opacity: 1, lineCap: 'round', lineJoin: 'round' }} />
          {/* Trazo blanco animado encima para máximo contraste */}
          <Polyline positions={caminoDijkstra}
            pathOptions={{ color: '#ffffff', weight: 2.5, dashArray: '1 14', opacity: 0.95, lineCap: 'round' }} />
        </>}

...
class OptimizadorGrafo:
    ...
    def ejecutar(self) -> dict:
        ...
        costo_transporte_total = 0.0

        for (u, v), arista in self.grafo.aristas.items():
            flujo = self._flujo_arista_original(flujo_dict, u, v)
            arista.flujo_actual = max(0.0, flujo)

            if flujo > 1e-6:
                flujos_optimos[f"{u}\u2192{v}"] = round(flujo, 4)
                costo_transporte_total += flujo * arista.costo_total_unitario
                rutas_activas.append({
                    ...
                    "distancia": round(arista.distancia, 2),
                    ...
                    "fuente_arista": arista.fuente_arista,
                })

        merma_total_estimada = 0.0
        for (u, _), arista in self.grafo.aristas.items():
            nodo_u = self.grafo.obtener_nodo(u)
            if (
                nodo_u is not None
        ...
        acopios_activos = []
        for acopio in self.acopios:
            flujo_entrante = float(
                flujo_dict.get(self._entrada(acopio.id), {}).get(
                    self._salida(acopio.id),
                    0.0,
                )
            )
        ...
        costo_penalizacion_calidad = 0.0
        for (u, _), arista in self.grafo.aristas.items():
            if u in set(self.acopios_penalizados):
                costo_penalizacion_calidad += (
                    arista.flujo_actual * config.PENALIZACION_CALIDAD
                )

        ganancia = self._calcular_ganancia(
            costo_operacion_total,
            costo_penalizacion_calidad,
        )

        logger.info(
            f"Optimizacion por grafos: {len(rutas_activas)} rutas activas, "
            f"costo={costo_transporte_total:.2f}, ganancia={ganancia:.2f}, "
        ...
        return {
            ...
            "costo_minimo": round(costo_transporte_total, 4),
            "costo_objetivo_optimizado": round(costo_objetivo, 4),
            ...
            "costo_penalizacion_calidad": round(costo_penalizacion_calidad, 4),
    ...
    def _calcular_ganancia(
        self,
        costo_operacion_total: float = 0.0,
        costo_penalizacion_calidad: float = 0.0,
    ) -> float:
        """Ingreso por demanda cubierta menos costos y penalizaciones."""
        ingreso = 0.0
        penalizacion_incumplimiento = 0.0

        for destino in self.destinos:
            recibido = sum(
                self.grafo.obtener_arista(u, destino.id).flujo_actual
                for u in self.grafo.vecinos_entrada(destino.id)
                if self.grafo.obtener_arista(u, destino.id)
            )
            ...
            penalizacion_incumplimiento += (
                max(0.0, destino.demanda - recibido)
                * config.PENALIZACION_INCUMPLIMIENTO
            )

        costo_transporte = sum(
            a.flujo_actual * a.costo_total_unitario for a in self.grafo.aristas.values()
        )
